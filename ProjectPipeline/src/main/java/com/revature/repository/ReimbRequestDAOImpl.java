...
public class ReimbRequestDAOImpl implements ReimbRequestDAO {
	...
	public List<ReimbRequest> findReimbRequestsByManagerId(int managerId) {
		...
		try {
	        ...
	        currentSession.beginTransaction();	   
	        /*
	         * Select * From project1.reimb_request as reibmReq 
	        	left join project1.empl_mgr as emgr on reibmReq.employeeId = emgr.employeeId 
	        	where emgr.managerId = 8 or reibmReq.employeeid =8 order by reibmReq.employeeid
	         */
	        String hql = "Select * From project1.reimb_request as reibmReq "+
	        				"left join project1.empl_mgr as emgr on reibmReq.employeeId = emgr.employeeId "+
	...
	public ReimbRequest findReimbRequestByRequestId(int requestId) {
		...
		try {
	        ...
	        currentSession.beginTransaction();	        	        	        	        
	        String hql = "Select * From project1.reimb_request as reibmReq "+
	        				"where reibmReq.reimbReqId =:requestId";
			Query<ReimbRequest> query = currentSession.createNativeQuery(hql,ReimbRequest.class);
			query.setParameter("requestId", requestId);
			reimbReq = (ReimbRequest)query.uniqueResult();
	        currentSession.getTransaction().commit();
	        */
	        currentSession = HibernateSessionFactory.getSession();
	        currentSession.beginTransaction();	        	        	        	        
	...
	/*
	public ReimbRequest findReimbRequestsByReimbId(int reimbId) {
		...
		try {
	        ...
	        currentSession.beginTransaction();	        	        	        	        
	        String hql = "Select * From project1.reimb_request as reibmReq "+
	        				"where reibmReq.requestId =:requestId";
			Query<ReimbRequest> query = currentSession.createNativeQuery(hql,ReimbRequest.class);
			query.setParameter("requestId", requestId);
			reimbReq = (ReimbRequest)query.uniqueResult();
	        currentSession.getTransaction().commit();
	...
	*/
	...
	public List<ReimbRequest> findReimbRequestsByManagerAndStatusId(int managerId, int statusId) {
		...
		try {
	         ...
	         * Select * From project1.reimb_request as reibmReq 
    			left join project1.empl_mgr as emgr on reibmReq.employeeId = emgr.employeeId 
    			where (emgr.managerId = 8 or reibmReq.employeeid =8) and reibmReq.statusId =2
	         */	        
	        String hql = "Select * From project1.reimb_request as reibmReq"
	        			+ "	left join project1.empl_mgr as emgr on reibmReq.employeeId = emgr.employeeId"
